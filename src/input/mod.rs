...
use crate::physics::particles::{spawn_particles, GravityStrength, Particle};
...
fn keyboard_input(
    ...
    if keys.just_pressed(KeyCode::Space) {
        spawn_particles(&mut commands, 20, half, PENTATONIC_FREQS);
    }
    ...
    if keys.just_pressed(KeyCode::Equal) || keys.just_pressed(KeyCode::NumpadAdd) {
        gravity.0 = (gravity.0 + 100.0).min(5000.0);
    }

    if keys.just_pressed(KeyCode::Minus) || keys.just_pressed(KeyCode::NumpadSubtract) {
        gravity.0 = (gravity.0 - 100.0).max(0.0);
    }
}
